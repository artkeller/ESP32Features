...
The table below summarizes the core features. For detailed analyses, see the following sections:

## Table of Contents

- [Overview table of ESP32 models](#overview-table-of-esp32-models)
- [Espressif SoC Product Portfolio](#espressif-soc-product-portfolio)
- [Deep Sleep](#deep-sleep)
- [Radio & Connectivity](#radio--connectivity)
- [Interface Speed Classes and Bus Characteristics](#interface-speed-classes-and-bus-characteristics)
- [Thorough Evaluation of Each Model](#thorough-evaluation-of-each-model)
...
|36. 4K/8K Video Streaming|-- (802.11 b/g/n, no HW video codec)|-- (DVP camera present, but Wi‑Fi b/g/n too slow, no H.264)|-- (camera/LCD present, but Wi‑Fi 4, no H.264 encoder)|-- (no camera interface, Wi‑Fi 4)|-- (no camera interface, minimal Wi‑Fi 4)|-- (dual‑band Wi‑Fi 6, but no video/camera interface)|-- (Wi‑Fi 6 2.4 GHz, but no video/camera interface)|-- (no Wi‑Fi)|- (H.264 encoder + MIPI CSI/DSI + JPEG – best video HW in the portfolio, but no integrated radio, requires a companion chip e.g. C6/E22)|++ (2.4 Gbps, 160 MHz)|-- (no Wi‑Fi, only BLE/802.15.4)|+ (Wi‑Fi 6 single‑band + JPEG codec/PPA, but no H.264 encoder – suitable for lower bitrates)|
|37. AR/VR Accessory|-- (no Wi‑Fi 6, higher power draw, higher latency)|-- (no BT, only Wi‑Fi 4)|- (BT5 LE + camera/LCD, but no Wi‑Fi 6)|- (BT5 LE, but no display/camera interface)|-- (minimal, no display/camera interface)|+ (dual‑band Wi‑Fi 6 + BT LE5, LP core – for low‑latency sensing, no display)|+ (Wi‑Fi 6 + BT 5.3, LP core – for controller/tracker use, no display)|- (only BT LE5.3, no Wi‑Fi – only simple haptic controllers)|- (MIPI CSI/DSI + high‑performance RISC‑V ideal for AR processing, but no integrated radio)|++ (low latency)|- (only BT LE + 802.15.4, no Wi‑Fi – only simple low‑power accessories)|+ (Wi‑Fi 6 + BT 5.4 + DVP camera/LCD, but single‑band – higher latency than tri‑band E22)|
|38. Wireless Network Adapter (M.2)|-- (no PCIe/SDIO host interface, standalone MCU)|-- (no PCIe/SDIO host interface)|-- (no PCIe/SDIO host interface)|-- (no PCIe/SDIO host interface)|-- (no PCIe/SDIO host interface)|-- (no PCIe/SDIO host interface)|-- (no PCIe/SDIO host interface)|-- (no PCIe/SDIO host interface)|-- (no PCIe/SDIO host interface)|++ (PCIe/SDIO)|-- (no PCIe/SDIO, only UART/SPI/I2C)|- (SDIO for SD/MMC storage present, but not designed as a PCIe network adapter)|
|39. Industrial Bridging|+ (Ethernet MAC + TWAI, established industrial ecosystem)|0 (no native TWAI/Ethernet, only USB OTG)|0 (TWAI present, but no Ethernet)|0 (TWAI present, but no Ethernet, mono‑core)|0 (no CAN/Ethernet, minimal)|+ (2× CAN FD + dual‑band Wi‑Fi 6, but no Ethernet)|+ (TWAI + Wi‑Fi 6, but no Ethernet)|0 (no CAN/Ethernet, only 802.15.4/BT)|++ (3× TWAI + Ethernet 10/100)|++ (robust Wi‑Fi 6E)|0 (no CAN/Ethernet, only BLE/802.15.4)|++ (Gigabit Ethernet + TWAI)|
|40. Set‑top Box / Laptop|-- (no PCIe host interface)|-- (no PCIe host interface)|-- (no PCIe host interface)|-- (no PCIe host interface)|-- (no PCIe host interface)|-- (no PCIe host interface)|-- (no PCIe host interface)|-- (no PCIe host interface)|-- (no PCIe host interface)|++ (PCIe/SDIO)|-- (no PCIe/SDIO)|- (SD/MMC interface present, but not designed as a PCIe Wi‑Fi module)|
|41. Matter‑over‑Thread Sensor|-- (no 802.15.4 radio)|-- (no 802.15.4 radio)|-- (no 802.15.4 radio)|-- (no 802.15.4 radio)|-- (no 802.15.4 radio)|+ (802.15.4 Zigbee 3.0/Thread 1.4, but dual‑band Wi‑Fi 6 raises cost/power draw for a pure sensor)|+ (802.15.4 Thread 1.3 + LP core, but higher power draw than H2/H21 due to the integrated Wi‑Fi 6 radio)|++ (802.15.4 Thread/Zigbee/Matter, 7 µA deep sleep, no Wi‑Fi overhead)|-- (no integrated radios)|- (no 802.15.4, only Wi‑Fi/BT as RCP)|++ (ESP‑Matter‑SDK, 5 µA)|++ (802.15.4 + Wi‑Fi 6, ESP‑Matter‑SDK expected – exact power figures not yet final)|
|42. Bluetooth‑Mesh Lighting|+ (BT LE present, but no native BT5 mesh feature)|-- (no BT)|+ (BT5 LE Mesh)|+ (BT5 LE)|+ (BT5 LE)|+ (BT LE5)|++ (BT5.3 Mesh native)|++ (BT LE5.3 Mesh/Long‑Range)|-- (no integrated radio)|+ (BT Classic + BLE5.4, but as an RCP not optimized for mesh lighting nodes)|++ (ESP‑BLE‑MESH, 20 dBm)|++ (BT 5.4 Mesh 1.1)|
|43. Occupancy Sensor|+ (ADC 18 ch. + radio, but ~10 µA deep sleep – higher than sensor‑class chips)|+ (ADC 20 + touch, but no BT/802.15.4)|+ (ADC 20 + touch/BT LE, higher power draw than C‑series)|++ (low‑power RISC‑V, ADC 6, ~5 µA deep sleep)|++ (ultra‑low‑cost/power, ADC 6)|++ (LP core + ADC 6, ~12 µA deep sleep)|++ (LP core + ADC 7, 7 µA deep sleep)|++ (ADC 5, 7 µA deep sleep)|+ (ADC present, but no dedicated low‑power radio, deep sleep ~0.025 mA)|- (no ADC channel, RCP not designed for standalone sensing)|++ (5 µA)|++ (ADC present, LP behavior expected, exact deep‑sleep figures not yet published)|
|**44. Large‑Scale GPIO Relay/Matrix Controller** *(new)*|+ (34 GPIOs + PWM + Ethernet/TWAI – solid all‑rounder with bus connectivity)|+ (43 GPIOs + LED PWM, but no CAN/Ethernet for bus integration)|++ (45 GPIOs + MCPWM, good channel count for larger relay matrices)|0 (only 22/16 GPIOs, TWAI present, but few pins for matrix applications)|- (only 14 GPIOs, too few for matrix applications)|+ (29 GPIOs + MCPWM 6 ch. + 2× CAN FD, moderate pin count)|+ (30/22 GPIOs + MCPWM 3 ch. + TWAI)|- (only 19 GPIOs, no CAN/Ethernet)|++ (55 GPIOs (16 LP) + MCPWM 2 + 3× TWAI + Ethernet – highest pin count and fieldbus connectivity in the portfolio)|-- (RCP design, GPIOs primarily reserved for the host interface, no PWM peripheral listed)|- (only 19 GPIOs, PWM present, but no CAN/Ethernet)|++ (60 GPIOs – highest count in the portfolio – + MCPWM + TWAI + Gigabit Ethernet)|
|**45. USB Video Class (UVC) Webcam / Capture Device** *(new)*|-- (no USB OTG, no camera interface)|++ (USB OTG FS + DVP camera present, Full‑Speed limits resolution/frame rate)|++ (USB OTG FS + camera + AI vector extensions for on‑chip processing)|-- (no USB OTG, only USB Serial/JTAG; no camera interface)|-- (no USB OTG, no camera interface)|-- (no USB OTG, no camera interface)|-- (no USB OTG, no camera interface)|-- (no USB OTG, no camera interface)|++ (USB HS/FS OTG + MIPI camera + hardware JPEG/H.264/ISP – highest resolution/frame rate in the portfolio)|-- (USB 2.0 serves only the host connection, no camera interface)|-- (no USB OTG, no camera interface)|++ (USB OTG + DVP camera + JPEG codec – full UVC suitability)|
|46. Smart Speaker / Voice Assistant|+ (dual‑core + I2S)|+ (I2S + USB)|++ (I2S + AI vector extensions)|0 (I2S, but mono‑core)|0 (I2S minimal)|+ (I2S + dual‑core)|+ (I2S + dual‑core)|+ (I2S)|++ (VAD + I2S/LP)|- (no I2S/local AI processing, RCP for connectivity, no audio front‑end)|- (I2S present, but no Wi‑Fi for cloud connectivity, mono‑core minimal)|++ (BT 5.4 LE Audio + I2S + AI/SIMD)|
|47. Industrial HMI|+ (I2S/SPI, but no LCD interface)|++ (LCD interface + 14× touch)|++ (LCD/camera + 14× touch)|0 (no LCD/touch)|- (minimal, no LCD/touch/Ethernet)|0 (no LCD/touch)|0 (no LCD/touch)|0 (no LCD/touch, no Ethernet)|++ (MIPI DSI + LCD + 14× touch + Ethernet)|- (no LCD/touch/camera interface, pure RCP)|- (no LCD/touch)|++ (LCD 24‑bit + 14× touch + Ethernet)|
|48. Security Camera with Edge‑AI|+ (Wi‑Fi/BT, but no camera interface)|++ (DVP camera + Wi‑Fi, but no AI extensions)|++ (camera + AI vector extensions)|0 (no camera interface)|- (no camera interface, minimal)|0 (no camera interface)|0 (no camera interface)|- (no Wi‑Fi/camera)|++ (MIPI CSI/ISP/JPEG/H.264 + AI/DSP)|- (no camera interface, pure RCP for connectivity)|- (no camera interface, no Wi‑Fi)|++ (DVP + JPEG + AI/SIMD)|
|49. Matter Gateway|+ (Wi‑Fi/BT, but no 802.15.4 – requires an external Thread radio)|+ (Wi‑Fi only, no 802.15.4/BT)|+ (Wi‑Fi/BT LE, no 802.15.4)|+ (Wi‑Fi/BT LE, no 802.15.4)|+ (Wi‑Fi/BT LE minimal, no 802.15.4)|++ (multi‑protocol Wi‑Fi 6 + 802.15.4)|++ (Wi‑Fi 6 + 802.15.4)|++ (802.15.4 Thread native, but no Wi‑Fi of its own – typically paired with a Wi‑Fi SoC)|- (no radios)|+ (tri‑band Wi‑Fi 6E, but no 802.15.4 – requires a companion chip, e.g. H21)|++ (802.15.4/BT, ESP‑Matter‑SDK – no Wi‑Fi, ideal as the Thread side of a dual‑chip gateway)|++ (Wi‑Fi 6 + 802.15.4 + Ethernet – all‑in‑one)|
|**50. Predictive‑Maintenance Vibration/Condition‑Monitoring Node** *(new)*|+ (ADC 18 ch. + dual‑core + Wi‑Fi/BT, but no LP core – higher power draw for battery operation)|0 (ADC 20 ch., but mono‑core and no BT for redundant telemetry)|++ (ADC 20 ch. + AI/vector extensions for on‑chip FFT/anomaly detection + Wi‑Fi/BT)|0 (ADC 6 ch., mono‑core RISC‑V, limited compute power for FFT)|- (ADC 6 ch., minimal, no LP core for continuous monitoring)|+ (ADC 6 ch. + dual‑core + LP core + dual‑band Wi‑Fi 6 – energy‑efficient for battery‑powered sensing)|+ (ADC 7 ch. + dual‑core + LP core + Wi‑Fi 6)|0 (ADC 5 ch., mono‑core, no Wi‑Fi – mesh telemetry only, no direct cloud connectivity)|++ (2× ADC + AI/DSP extensions + up to 360 MHz for complex FFT/ML models, but no integrated radio – requires a companion chip, e.g. P4+C6, see Combinations)|-- (no ADC channel, RCP design not intended for sensor data acquisition)|+ (ADC present [channel count unspecified] + 5 µA deep sleep – suitable for simple battery‑powered vibration sensing, but no LP core for complex on‑site FFT)|+ (ADC present [channel count unspecified] + 128‑bit SIMD for signal processing, deep‑sleep figures not yet published)|
...
* **ESP32 Series Datasheet** (Version 5.3, accessed August 2026)  
  [https://documentation.espressif.com/esp32_datasheet_en.pdf](https://documentation.espressif.com/esp32_datasheet_en.pdf) *(the `www.espressif.com/sites/default/files/...` mirror still resolves as of this writing, but `documentation.espressif.com` is Espressif's current canonical documentation host)*

* **ESP32‑S2 Series Datasheet** (Version 1.9, accessed August 2026)  
...
* **ESP32‑C6 Series Datasheet** (Version 1.5, accessed August 2026)  
  [https://documentation.espressif.com/esp32-c6_datasheet_en.pdf](https://documentation.espressif.com/esp32-c6_datasheet_en.pdf)

* **ESP32‑C61 Series Datasheet** (Version 1.1, accessed August 2026)  
  [https://documentation.espressif.com/esp32-c61_datasheet_en.pdf](https://documentation.espressif.com/esp32-c61_datasheet_en.pdf) *(previously listed as v1.0; current revision is v1.1)*

* **ESP32‑H2 Series Datasheet** (Version 1.3, accessed August 2026)  
  [https://documentation.espressif.com/esp32-h2_datasheet_en.pdf](https://documentation.espressif.com/esp32-h2_datasheet_en.pdf) *(the previous "Version 1.0/1.5" citation did not match any published revision; the current datasheet revision is v1.3)*

* **ESP32‑H4 – Product Page** (accessed August 2026)  
...
* **ESP32‑H21 – Product Page** (accessed August 2026)  
