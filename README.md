...
|36. 4K/8K Video Streaming|-- (802.11 b/g/n, kein HW‑Videocodec)|-- (DVP‑Kamera vorhanden, aber Wi‑Fi b/g/n zu langsam, kein H.264)|-- (Kamera/LCD vorhanden, aber Wi‑Fi 4, kein H.264‑Encoder)|-- (kein Kamera‑Interface, Wi‑Fi 4)|-- (kein Kamera‑Interface, minimales Wi‑Fi 4)|-- (Dual‑Band Wi‑Fi 6, aber kein Video‑/Kamera‑Interface)|-- (Wi‑Fi 6 2,4 GHz, aber kein Video‑/Kamera‑Interface)|-- (kein Wi‑Fi)|- (H.264‑Encoder + MIPI CSI/DSI + JPEG – bestes Video‑HW im Portfolio, aber kein integriertes Funkmodul, benötigt Companion‑Chip z. B. C6/E22)|++ (2.4 Gbps, 160 MHz)|-- (kein Wi‑Fi, nur BLE/802.15.4)|+ (Wi‑Fi 6 Single‑Band + JPEG‑Codec/PPA, aber kein H.264‑Encoder – für geringere Bitraten geeignet)|
|37. AR/VR Accessory|-- (kein Wi‑Fi 6, hoher Verbrauch, höhere Latenz)|-- (kein BT, nur Wi‑Fi 4)|- (BT5 LE + Kamera/LCD, aber kein Wi‑Fi 6)|- (BT5 LE, aber kein Display‑/Kamera‑Interface)|-- (minimal, kein Display‑/Kamera‑Interface)|+ (Dual‑Band Wi‑Fi 6 + BT LE5, LP‑Core – für Low‑Latency‑Sensorik, kein Display)|+ (Wi‑Fi 6 + BT 5.3, LP‑Core – für Controller/Tracker, kein Display)|- (nur BT LE5.3, kein Wi‑Fi – nur einfache Haptik‑Controller)|- (MIPI CSI/DSI + High‑Perf RISC‑V ideal für AR‑Verarbeitung, aber kein integriertes Funkmodul)|++ (Low‑Latency)|- (nur BT LE + 802.15.4, kein Wi‑Fi – nur einfaches Low‑Power‑Zubehör)|+ (Wi‑Fi 6 + BT 5.4 + DVP‑Kamera/LCD, aber Single‑Band – höhere Latenz als Tri‑Band E22)|
|38. Wireless Network Adapter (M.2)|-- (kein PCIe/SDIO‑Host‑Interface, Standalone‑MCU)|-- (kein PCIe/SDIO‑Host‑Interface)|-- (kein PCIe/SDIO‑Host‑Interface)|-- (kein PCIe/SDIO‑Host‑Interface)|-- (kein PCIe/SDIO‑Host‑Interface)|-- (kein PCIe/SDIO‑Host‑Interface)|-- (kein PCIe/SDIO‑Host‑Interface)|-- (kein PCIe/SDIO‑Host‑Interface)|-- (kein PCIe/SDIO‑Host‑Interface)|++ (PCIe/SDIO)|-- (kein PCIe/SDIO, nur UART/SPI/I2C)|- (SDIO für SD/MMC‑Storage vorhanden, aber nicht als PCIe‑Netzwerkadapter konzipiert)|
|39. Industrial Bridging|+ (Ethernet MAC + TWAI, etabliertes Industrie‑Ökosystem)|0 (kein natives TWAI/Ethernet, nur USB OTG)|0 (TWAI vorhanden, aber kein Ethernet)|0 (TWAI vorhanden, aber kein Ethernet, Mono‑Core)|0 (kein CAN/Ethernet, minimal)|+ (2× CAN FD + Dual‑Band Wi‑Fi 6, aber kein Ethernet)|+ (TWAI + Wi‑Fi 6, aber kein Ethernet)|0 (kein CAN/Ethernet, nur 802.15.4/BT)|++ (3× TWAI + Ethernet 10/100)|++ (Robust Wi‑Fi 6E)|0 (kein CAN/Ethernet, nur BLE/802.15.4)|++ (Gigabit Ethernet + TWAI)|
|40. Set‑top Box / Laptop|-- (kein PCIe‑Host‑Interface)|-- (kein PCIe‑Host‑Interface)|-- (kein PCIe‑Host‑Interface)|-- (kein PCIe‑Host‑Interface)|-- (kein PCIe‑Host‑Interface)|-- (kein PCIe‑Host‑Interface)|-- (kein PCIe‑Host‑Interface)|-- (kein PCIe‑Host‑Interface)|-- (kein PCIe‑Host‑Interface)|++ (PCIe/SDIO)|-- (kein PCIe/SDIO)|- (SD/MMC‑Interface vorhanden, aber kein PCIe‑WLAN‑Modul‑Design)|
